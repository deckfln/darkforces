...
bool dfMesh::collisionSegmentTriangle(const glm::vec3& p, const glm::vec3& q, std::list<gaCollisionPoint>& collisions, int firstVertex, int nbVertices)
{
	...
	for (unsigned int i = firstVertex; i < nbVertices; i += 3) {
		if (Framework::IntersectSegmentTriangle(p, q,
			...
			collisions.push_back(gaCollisionPoint(fwCollisionLocation::BACK, collision, &m_vertices[i]));
		};
	}
...
bool dfMesh::checkCollision(fwCylinder& bounding, glm::vec3& direction, glm::vec3& intersection, std::string& name, std::list<gaCollisionPoint>& collisions)
{
	...
	for (unsigned int i = 0; i < m_vertices.size(); i += 3) {
		...
		if (Framework::intersectSphereTriangle(center_es, v1_es, v2_es, v3_es, origin)) {
			...
			if (delta > 0.9) {
				collisions.push_back(gaCollisionPoint(fwCollisionLocation::BOTTOM, intersection, &m_vertices[i]));
				c = fwCollisionLocation::BOTTOM;
			}
			else if (delta < -0.9) {
				collisions.push_back(gaCollisionPoint(fwCollisionLocation::TOP, intersection, &m_vertices[i]));
				c = fwCollisionLocation::TOP;
			}
			else if (delta > 0.5 && d > 0) {
				collisions.push_back(gaCollisionPoint(fwCollisionLocation::FRONT_BOTTOM, intersection, &m_vertices[i]));
				c = fwCollisionLocation::FRONT_BOTTOM;
			}
			else if (delta < -0.5 && d > 0) {
				collisions.push_back(gaCollisionPoint(fwCollisionLocation::FRONT_TOP, intersection, &m_vertices[i]));
				c = fwCollisionLocation::FRONT_TOP;
			}
			else if (d <= 0) {
				collisions.push_back(gaCollisionPoint(fwCollisionLocation::BACK, intersection, &m_vertices[i]));
				c = fwCollisionLocation::BACK;
			}
			else if (d <= 0.4) {
				collisions.push_back(gaCollisionPoint(fwCollisionLocation::LEFT, intersection, &m_vertices[i]));
				c = fwCollisionLocation::LEFT;
			}
			else {
				float l = glm::length(intersection - center_ws);
				float l1 = glm::length(origin - center_es);
				collisions.push_back(gaCollisionPoint(fwCollisionLocation::FRONT, intersection, &m_vertices[i]));
